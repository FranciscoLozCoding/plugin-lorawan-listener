...
def parse_loriot_payload(body, websocket_url=None):
    """
    Parse Loriot uplink JSON. Requires decoded payload in 'decoded.data' or 'object'.
    When websocket_url is provided, measurement_metadata['lns'] is set to the URL host (e.g. us1.loriot.io).

    Returns dict with keys: measurements, timestamp_ns, measurement_metadata, signal_values, signal_metadata,
    or None if decoded payload is missing.
    """
    try:
    ...
    if not payload or not isinstance(payload, dict):
        logging.debug("Loriot: no decoded.data/object; skipping")
        return None
